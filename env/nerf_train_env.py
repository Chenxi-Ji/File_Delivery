...
print(f"Poses shape: {envs.shape}")
...
envs = torch.Tensor(data['env']).to(device)
...
testenv = torch.Tensor(envs).to(device)
...
chunksize = 2**18  # Modify as needed to fit in GPU memory
...
def init_models():
    ...
    encoder_env = PositionalEncoderEnv(2, n_freqs, log_space=log_space)
    ...
    # Models
    model = NeRF(
        encoder.d_output+encoder_env.d_output,
        n_layers=n_layers,
        d_filter=d_filter,
        skip=skip,
    ...
    return model, fine_model, encode, encode_viewdirs,encode_env, optimizer, warmup_stopper
...
def train():
    ...
    if not one_image_per_step:
        ...
        envs_rgb = torch.stack([envs[i].repeat(height, width, 1) for i in range(n_training)], 0)
        rays_rgb = torch.cat([all_rays, images[:, None]], 1)
        rays_rgb = torch.permute(rays_rgb, [0, 2, 3, 1, 4])
        print('rays_rgb.shape:',rays_rgb.shape)
        print('envs_rgb.shape:',envs_rgb.shape)
        rays_rgb = rays_rgb.reshape([-1, 3, 3])
        rays_rgb = rays_rgb.reshape([rays_rgb.shape[0],9])
        envs_rgb = envs_rgb.reshape([-1,2])
        rays_rgb = torch.cat([rays_rgb,envs_rgb],1)
    ...
    for i in trange(n_iters):
        ...
        if one_image_per_step:
            ...
            target_env = envs[target_img_idx].to(device)
            rays_o, rays_d = get_rays(height, width, focal, target_pose)
            rays_o = rays_o.reshape([-1, 3])
            rays_d = rays_d.reshape([-1, 3])
            rays_env = target_env.repeat(rays_o.shape[0],1)
        else:
            # Random over all images.
            batch_all = rays_rgb[i_batch : i_batch + batch_size]
            rays_env = batch_all[:,9:11]
        ...
        if i%100==0:
            feature="env_"+str(dataname)+"_"+str(n_freqs)+"_"+str(n_freqs_views)+"_"+str(d_filter)+"_"+str(n_layers)+"_"+str(n_iters)#+"_"+str(n_samples)
...
print("dataname,n_iters:",dataname,n_iters)

# Run training session(s)
for _ in range(n_restarts):
    model, fine_model, encode, encode_viewdirs, encode_env, optimizer, warmup_stopper = (
        init_models()
    )
