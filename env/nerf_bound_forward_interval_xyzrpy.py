...
def compute_image_sampling(ray_model,directions,input,input_type,num_sampling,xyz_step, ry_step,\
...
def compute_image_bound(ray_model,input,input_type,num_sampling,xyz_step,ry_step,\
    ...
    for start_height in range(start_vis_height,end_vis_height,tile_height):
        for start_width in range(start_vis_width,end_vis_width,tile_width):
            ...
            ls,us=compute_image_sampling(ray_model,directions,input,input_type,num_sampling,xyz_step, ry_step,\
                           start_height,end_height,start_width,end_width)
...
if __name__ == "__main__":
    ...
    n_iters=100000
    chunksize = 2**5
    xyz_step,ry_step=0.0005,0.00015
    ...
    if input_type in ["x", "y", "z","roll", "yaw"]:
        ...
        for cur_val in tqdm(vals):
            ...
            image_lb, image_ub,image_ls,image_us  = compute_image_bound(ray_model, input_tensor, input_type,num_sampling, xyz_step, ry_step,
        ...
        for cur_roll, cur_yaw in tqdm(itertools.product(roll_vals, yaw_vals)):
            ...
            image_lb,image_ub,image_ls,image_us=compute_image_bound(ray_model,input,input_type,num_sampling, xyz_step,ry_step,\
                                start_vis_height,end_vis_height,tile_height,start_vis_width,end_vis_width,tile_width,\
                                print_flag,visual_flag,device)
    ...
    # print(len(images_ls),images_ls[0].shape,len(images_us),images_us[0].shape)
