...
class PositionalEncoderEnv(nn.Module):
    ...
    # def forward(self, x) -> torch.Tensor:
    #     r"""
    #     Apply positional encoding to environmental inputs.
    #     """
...
class TestModel(nn.Module):
    ...
    def update_xyzrpy(self, x, y, z, roll, pitch, yaw):
        self.x = x
        self.y = y
        self.z = z
        self.roll = roll
    ...
    def get_extrinsic_matrix(self, xyzrpy):
        x = xyzrpy[:, 0:1] if self.x is None else self.x.to(xyzrpy.device)*torch.ones_like(xyzrpy[:,0:1])
        y = xyzrpy[:, 1:2] if self.y is None else self.y.to(xyzrpy.device)*torch.ones_like(xyzrpy[:,1:2])
        z = xyzrpy[:, 2:3] if self.z is None else self.z.to(xyzrpy.device)*torch.ones_like(xyzrpy[:,2:3])
        
        roll = xyzrpy[:, 3:4] if self.roll is None else self.roll.to(xyzrpy.device)*torch.ones_like(xyzrpy[:,3:4])
        pitch = xyzrpy[:, 4:5] if self.pitch is None else self.pitch.to(xyzrpy.device)*torch.ones_like(xyzrpy[:,4:5])
        yaw = xyzrpy[:, 5:6] if self.yaw is None else self.yaw.to(xyzrpy.device)*torch.ones_like(xyzrpy[:,5:6])

        R00 = torch.cos(yaw) * torch.cos(pitch)
        R01 = torch.cos(yaw) * torch.sin(pitch) * torch.sin(roll) - torch.sin(yaw) * torch.cos(roll)
        R02 = torch.cos(yaw) * torch.sin(pitch) * torch.cos(roll) + torch.sin(yaw) * torch.sin(roll)
        R03 = x

        R10 = torch.sin(yaw) * torch.cos(pitch)
        R11 = torch.sin(yaw) * torch.sin(pitch) * torch.sin(roll) + torch.cos(yaw) * torch.cos(roll)
        R12 = torch.sin(yaw) * torch.sin(pitch) * torch.cos(roll) - torch.cos(yaw) * torch.sin(roll)
        R13 = y

        R20 = -torch.sin(pitch)
        R21 = torch.cos(pitch) * torch.sin(roll)
        R22 = torch.cos(pitch) * torch.cos(roll)
        R23 = z
    ...
    def nerf_forward(
        ...
        # Prepare batches.
        batches = self.prepare_chunks(query_points, encoding_fn,rays_env, env_encoding_fn,rays_d)
    ...
    def forward(self,x):
        ...
        # self.start_height, self.end_height, self.start_width, self.end_width = (
        #     h_w[0,0], h_w[0,1], h_w[0,2], h_w[0,3]
        # )
        rays_env=x[..., 6:8]
        x=x[..., :6]
        ...
        # print('rays_o.shape:',rays_o.shape)
        # print('rays_d.shape:',rays_d.shape)
        # print('rays_env.shape:',rays_env.shape)
...
def save_data(save_path,image_lb,image_ub,image_noptb):
    np.savez(save_path, image_lb=image_lb,image_ub=image_ub,image_noptb=image_noptb)
...
if __name__ == "__main__":
    ...
    print("dataname,eps,hue_eps,satur_eps:",dataname,eps,hue_eps,satur_eps)
    print('ptb_type:',ptb_type)
    ...
    xyzrpy_org=xyzrpy = extrinsic_matrix_to_xyzrpy(testpose)
    print('xyzrpy:',xyzrpy)
    ...
    if input_type=="xyzrpy":
        dummy_inputpos = BoundedTensor(torch.rand((1, 8), device=device))
    elif input_type=="extrinsic_matrix":
        dummy_inputpos = BoundedTensor(torch.rand((1, 4,4), device=device))
    ...
    delta=1e-9
    if ptb_type=='xyzrpy':
        ray_model.update_xyzrpy(None,None,None,None,xyzrpy[4],None)
        eps_tensor=torch.tensor([[eps,eps,eps,eps,delta,eps,hue_eps,satur_eps]]).to(device)
    elif ptb_type=='env':
        ray_model.update_xyzrpy(None,xyzrpy[1],xyzrpy[2],xyzrpy[3],xyzrpy[4],xyzrpy[5])
        eps_tensor=torch.tensor([[delta,delta,delta,delta,delta,delta,hue_eps,satur_eps]]).to(device)
    elif ptb_type=='xy':
        ray_model.update_xyzrpy(None,None,xyzrpy[2],xyzrpy[3],xyzrpy[4],xyzrpy[5])
        eps_tensor=torch.tensor([[eps,eps,delta,delta,delta,delta,hue_eps,satur_eps]]).to(device)
    elif ptb_type=='xyz':
        ray_model.update_xyzrpy(None,None,None,xyzrpy[3],xyzrpy[4],xyzrpy[5])
        eps_tensor=torch.tensor([[eps,eps,eps,delta,delta,delta,hue_eps,satur_eps]]).to(device)
    elif ptb_type=='rpy':
        ray_model.update_xyzrpy(xyzrpy[0],xyzrpy[1],xyzrpy[2],None,xyzrpy[4],None)
        eps_tensor=torch.tensor([[delta,delta,delta,eps,delta,eps,hue_eps,satur_eps]]).to(device)
    ...
    #     for start_width in range(start_vis_width,end_vis_width,tile_width):
            ...
            if input_type=="xyzrpy":
                inputvec=torch.cat((xyzrpy,testenv),dim=0)
                inputpose= inputvec.repeat((end_height-start_height)*(end_width-start_width),1)
                xl=(inputvec-eps_tensor).repeat((end_height-start_height)*(end_width-start_width),1)
                xu=(inputvec+eps_tensor).repeat((end_height-start_height)*(end_width-start_width),1)
                # print(inputpose.shape)
            elif input_type=="extrinsic_matrix":
                inputpose=extrinsic_matrix.repeat((end_height-start_height)*(end_width-start_width),1,1)
            
            #print(inputpose.shape)
            exp=rgb_model(ray_model(inputpose,))
            ...
            model = BoundedModule(ray_model, (dummy_inputpos, ))
            ...
            # print('\n')
            # if (lb_ibp<=ub_ibp).all():
            #     print('ibp:yes')
            # else:
            #     print('ibp:no')
            backward_start_time = time.time()
            alpha_rgb_lb, alpha_rgb_ub = model.compute_bounds(
                x=(inputpose_ptb, ),
                method="backward",
                reference_bounds=reference_interm_bounds)
            
            # if (alpha_rgb_lb<=alpha_rgb_ub).all():
            #     print('alpha_rgb:yes')
            # else:
            #     print('alpha_rgb:no')
            #     tmp=alpha_rgb_ub-alpha_rgb_lb
   
                # print("tmp_min,tmp_max:",torch.min(tmp),torch.max(tmp))
                # print("color: tmp_min,tmp_max:",torch.min(tmp[...,:3]),torch.max(tmp[...,:3]))
                # print("dense: tmp_min,tmp_max:",torch.min(tmp[...,3]),torch.max(tmp[...,3]))
                # del tmp
            ...
            del exp, ptb, inputpose_ptb,lb_ibp, ub_ibp,reference_interm_bounds,\
                alpha_rgb_lb, alpha_rgb_ub,ptb_alpha_rgb,bounded_alpha_rgb,lb, ub, lirpa_rgb_model, model
    ...
    if bound_whole_flag:
        output_feature="env_"+str(dataname)+"_err_"+str(hue_eps).split(".")[1]+"_"+str(satur_eps).split(".")[1]+str(bound_method)+\
                    "_features_"+str(d_filter)+"_"+str(n_layers)+"_"+str(n_iters)+\
                    "_samples_"+str(n_samples)+"_ptbtype_"+ptb_type+\
                    "_xd_"+str(xdown_factor)+"_yd_"+str(ydown_factor)
            
        save_output_path='result/'+output_feature+'.npz'
        save_data(save_output_path,image_lb,image_ub,image_noptb)
    ...
    if visual_flag:
        ...
        if bound_whole_flag:
            imagename="bound_img_env_"+str(dataname)+"_err_"+str(hue_eps).split(".")[1]+"_"+str(satur_eps).split(".")[1]+str(bound_method)+\
                        "_features_"+str(d_filter)+"_"+str(n_layers)+"_"+str(n_iters)+\
                        "_samples_"+str(n_samples)+"_ptbtype_"+ptb_type+\
                        "_xd_"+str(xdown_factor)+"_yd_"+str(ydown_factor)+"_wh.png"
        else:
            imagename="region_img_env_"+str(dataname)+"_err_"+str(hue_eps).split(".")[1]+"_"+str(satur_eps).split(".")[1]+str(bound_method)+\
                        "_features_"+str(d_filter)+"_"+str(n_layers)+"_"+str(n_iters)+\
                        "_samples_"+str(n_samples)+"_ptbtype_"+ptb_type+\
                        "_yd_"+str(ydown_factor)+"_xd_"+str(xdown_factor)+\
