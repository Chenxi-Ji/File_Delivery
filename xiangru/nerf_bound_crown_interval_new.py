...
if __name__ == "__main__":
    ...
    dummy_inputpos = BoundedTensor(torch.rand((1, 6), device=device))
    ...
    #     for start_width in range(start_vis_width,end_vis_width,tile_width):
            ...
            #print(inputpose.shape)
            exp=rgb_model(ray_model(inputpose, h_w))

            ptb = PerturbationLpNorm(norm=np.inf, eps=eps)
            inputpose_ptb = BoundedTensor(inputpose, ptb)

            model = BoundedModule(ray_model, (dummy_inputpos, h_w))

            #print("computing ibp and crown")
            if not bound_whole_flag:
                print("Start IBP")
            lb_ibp, ub_ibp = model.compute_bounds(x=(inputpose_ptb, h_w), method="ibp")
            if not bound_whole_flag:
                print("IBP finished")
