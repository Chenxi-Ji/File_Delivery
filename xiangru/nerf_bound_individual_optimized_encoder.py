...
class TestModel(nn.Module):
    ...
    def get_rgb_map_backup(self,alpha:torch.Tensor, rgb:torch.Tensor)-> torch.Tensor:
...
if __name__ == "__main__":
    ...
    # ttt = torch.rand((1, 32, 3), device=device)

    encode = PositionalEncoder(d_input, n_freqs, log_space=log_space)
    encode.to(device)
    
    encode_viewdirs = PositionalEncoder(d_input, n_freqs_views, log_space=log_space)
    encode_viewdirs.to(device)
    ...
    for start_height in range(start_vis_height,end_vis_height,tile_height):
        for start_width in range(start_vis_width,end_vis_width,tile_width):
            ...
            # bounded_alpha = BoundedTensor(torch.randn((9, 32), device=device), ptb)
            # bounded_rgb = BoundedTensor(torch.randn((9, 32, 3), device=device), ptb)

            # model = BoundedModule(ray_model, (bounded_alpha, bounded_rgb))

            # print("Start test_backward")
            # test_backward_start_time = time.time()
            # profiler.start()
            # lb_test, ub_test = model.compute_bounds(x=(bounded_alpha, bounded_rgb), method="forward")
            # profiler.stop()
            # with open("nerf_bound_only_rgb_profile_forward.html", "w", encoding="utf-8") as f:
            #     f.write(profiler.output_html())
            # print("test_backward time:", time.time()-test_backward_start_time)
