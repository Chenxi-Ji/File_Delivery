...
class PositionalEncoderDebug(nn.Module):
    r"""
    Sine-cosine positional encoder for input points.
    """
...
class PositionalEncoderOri(nn.Module):
    r"""
    Sine-cosine positional encoder for input points.
    """
...
if __name__ == "__main__":
    ...
    encode.to(device)
    # result_new = encoder(ttt)

    # encoder_ori = PositionalEncoderOri(d_input, n_freqs, log_space=log_space)
    # encoder_ori.to(device)
    # result_ori = encoder_ori(ttt)

    # encoder_debug = PositionalEncoderDebug(d_input, n_freqs, log_space=log_space)
    # encoder_debug.to(device)
    # result_debug = encoder_debug(ttt)

    # lirpa_encoder = BoundedModule(encoder, torch.rand(9, 32, 3))
