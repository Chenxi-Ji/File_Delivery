...
if __name__ == "__main__":
    ...
    n_iters=100000
    chunksize = 2**4
    angle_start,angle_end=0.0, 0.01
    eps=0.002
    angle_step=0.002
    # cur_angle=angle=0.2
    ...
    dist=float(np.linalg.norm(xyzrpy_np[:2],ord=2))
    print("dist_val:",dist)
    init_angle=float(np.arctan2(xyzrpy_np[1], xyzrpy_np[0]))
    print("init_angle:",init_angle)
    print("x_val,y_val:",dist*np.cos(init_angle),dist*np.sin(init_angle))
    print("x_val,y_val:",dist*np.cos(init_angle+angle_step),dist*np.sin(init_angle+angle_step))
